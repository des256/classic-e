...
impl Window {
    ...
    pub fn begin_paint(&self) {
        unsafe { glXMakeCurrent(self.system.connection.get_raw_dpy(),self.id,self.system.context) };
    ...
    pub fn end_paint(&self) {
        unsafe { gl::Flush() };
        unsafe { glXSwapBuffers(self.system.connection.get_raw_dpy(),self.id) };
    }
}
