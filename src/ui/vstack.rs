...
impl VStack {
    pub fn new_from_vec(_state: &Rc<ui::UIState>,widgets: Vec<Box<dyn ui::Widget>>) -> VStack {
        VStack {
    ...
    fn set_rect(&self,r: Rect<i32>) {
        ...
        for widget in self.widgets.iter() {
            ...
            let (ox,sx) = match self.halign {
                ui::HAlignment::Left => { (r.ox(),size.x()) },
                ui::HAlignment::Right => { (r.ox() + r.sx() - size.x(),size.x()) },
