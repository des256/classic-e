...
fn main() -> Result<(),SystemError> {
    ...
    let vstack = Rc::new(VStack::new_from_vec(&ui,vec![text1,text2,text3,text4,text5,text6])?);
    vstack.halign.set(HAlignment::Center);

    // create texts for HStack
    ...
    // create HStack
    let hstack = Rc::new(HStack::new_from_vec(&ui,vec![text8,text9,text10,text11])?);
    hstack.valign.set(VAlignment::Center);

    let id1 = ui.open_frame(rect!(50,50,640,350),"VStack Test",&vstack);
    let id2 = ui.open_frame(rect!(150,150,640,350),"HStack Test",&hstack);
