...
impl Book {
    pub fn new_from_vec(state: &Rc<UIState>,pages: Vec<Page>) -> Result<Book,SystemError> {
        Ok(Book {
...
impl Widget for Book {
    ...
    fn draw(&self) {
        ...
        for i in 0..self.pages.len() {
            ...
            self.state.draw_rectangle(tab_rect,tc,BlendMode::Replace);
            self.state.draw_text(tab_rect.o() + inner_padding,&page.name,styles.book_tab_text_color,&styles.font);
            tab_rect.set_ox(tab_rect.ox() + tab_rect.sx());
        }
        self.state.draw_rectangle(rect!(
            tab_rect.o(),
            vec2!(
