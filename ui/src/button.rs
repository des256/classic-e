...
impl Button {
    pub fn new(state: &Rc<UIState>,text: &str,font: &Rc<Font>) -> Button {
        Button {
...
impl Widget for Button {
    ...
    fn draw(&self,context: Vec2<i32>) {
        ...
        self.state.draw_rectangle(Rect::<i32>::new_os(local_context + self.padding,self.r.get().s() - 2 * self.padding),bc,BlendMode::Replace);
        self.state.draw_text(local_context + self.padding + self.inner_padding,&self.text,self.color,&self.font);
    }
