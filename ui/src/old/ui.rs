...
pub struct WidgetWindow {
    ...
    pub has_mouse: Cell<bool>,
}

/// UI engine.
pub struct UI {
    pub state: Rc<UIState>,
    pub windows: RefCell<Vec<WidgetWindow>>,
    pub new_windows: RefCell<Vec<WidgetWindow>>,
    pub drop_windows: RefCell<Vec<usize>>,
}
...
impl UI {
    pub fn new(system: &Rc<System>,graphics: &Rc<Graphics>,font_dir: &str) -> Result<UI,SystemError> {
        ...
        Ok(UI {
            ...
            new_windows: RefCell::new(Vec::new()),
            drop_windows: RefCell::new(Vec::new()),
        })
    }
    ...
    pub fn open_frame<T: Widget + 'static>(&self,r: Rect<i32>,title: &str,widget: &Rc<T>) -> u64 {

        // create new frame
        let window = PlatformWindow::new_frame(&self.state.system,r,title);
        let id = window.id;

        // set widget rectangle accordingly
        let widget = Rc::clone(widget);
        widget.set_rect(Rect::<i32>::new_os(Vec2::<i32>::zero(),r.s()));

        // append newly created frame to new window list
        let mut new_windows = self.new_windows.borrow_mut();
        new_windows.push(WidgetWindow {
            state: Rc::clone(&self.state),
            window: window,
            widget: widget,
            has_mouse: Cell::new(false),
        });

        id
    }

    /// Open popup with a widget.
    /// 
    /// **Arguments**
    /// * `r` - Rectangle of the popup.
    /// * `widget` - The widget interface to run here.
    /// 
    /// **Returns**
    /// Unique ID for this popup.
    pub fn open_popup<T: Widget + 'static>(&self,r: Rect<i32>,id: u64,widget: &Rc<T>) -> u64 {

        // create new popup
        let window = PlatformWindow::new_popup(&self.state.system,id,r);
        let id = window.id;
        ...
        // append newly created popup to new window list
        let mut new_windows = self.new_windows.borrow_mut();
        new_windows.push(WidgetWindow {
            state: Rc::clone(&self.state),
            window: window,
    ...
    pub fn close(&self,id: u64) {
        // add window to the drop list
        let windows = self.windows.borrow();
        for i in 0..windows.len() {
            if windows[i].window.id == id {
                self.drop_windows.borrow_mut().push(i);
                break;
            }
        }
    }
    ...
    pub fn run(&self) {
        ...
        while self.state.running.get() {
            ...
            // remove closed windows from main list
            {
                let mut windows = self.windows.borrow_mut();
                let mut drop_windows = self.drop_windows.borrow_mut();
                drop_windows.sort_unstable();
                for i in drop_windows.iter().rev() {
                    windows.remove(*i);
                }
                drop_windows.clear();
            }

            // append newly created windows to the main list
            {
                let mut windows = self.windows.borrow_mut();
                let mut new_windows = self.new_windows.borrow_mut();
                windows.append(&mut new_windows);
            }
        }
    }
}

impl EventHandler for WidgetWindow {
    fn handle(&self,event: Event) {
        match event {
            ...
            Event::MouseMove(p) => {
                let new_has_mouse = self.widget.handle_mouse_move(p);
                if new_has_mouse != self.has_mouse.get() {
                    if new_has_mouse {
                        self.state.system.capture_mouse(self.window.id);
                    }
                    else {
                        self.state.system.release_mouse();
                    }
                    self.has_mouse.set(new_has_mouse);
                }
            },
        }
    }
